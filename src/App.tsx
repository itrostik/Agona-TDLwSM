...
import { useStore } from "../zustand/store/useStore.ts";
...
function App() {
  ...
  const addTaskFn = useStore((state) => state.add);
  const removeTaskFn = useStore((state) => state.remove);
  const doneTaskFn = useStore((state) => state.done);
  const undoneTaskFn = useStore((state) => state.undone);
